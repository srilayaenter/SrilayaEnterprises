...
\n**Add New Product (Enhanced with Vendor Details Integration)**
...
- Form fields: Product Name, Category (dropdown: Rice/Flour/Flakes/Millets/Honey/Custom Categories - **admin can only select from existing categories, cannot add new categories here**), Cost Price (₹/kg), Selling Price (auto-calculated as Cost Price + 25%), GST percentage\n- **Vendor Selection Section:**
  + **Vendor Assignment Option:** Radio buttons to choose between Existing Vendor or New Vendor
  + **For Existing Vendor:**
    * Vendor dropdown: Select from registered vendors in the system
    * Display vendor details: Contact Person, Phone, Email (read-only)
    * Option to view full vendor profile (link to Vendor Management)
  + **For New Vendor:**
    * Inline vendor registration form with fields:\n      - Vendor Name (required)
      ...
      - GST Number (optional)
    * Save Vendor button: Creates new vendor record and links to product\n    * Cancel button: Returns to Existing Vendor selection
  + **Vendor-Product Linking:** System automatically creates association between selected/new vendor and product upon saving
- Image upload functionality with preview
- **Packaging Category Selection:**
  + **Packaging options selection with initial stock quantity input:**
    * For standard categories (Rice, Flour, Flakes, Millets): Checkboxes for1kg, 2kg, 5kg, 10kg
...
- **Post-Save Confirmation:** Success message displays product details and linked vendor information

**View Product List (Enhanced with Vendor Information)**
- **Back Button:** Navigate back to Admin Dashboard
- Table display with columns: Product ID, Product Name, Category, **Vendor Name**, Cost Price, Selling Price, GST%, Final Price, Stock Status, Actions
- Pagination with 20 products per page
- Search bar for product name, ID, or vendor name
- Filter dropdowns: Category (including custom categories), **Vendor**, Stock Status (In Stock/Low Stock/Out of Stock)
- Sort options: Name (A-Z), Price (Low to High), Recently Added, **Vendor Name (A-Z)**
\n**Edit Product (Enhanced with Vendor Management)**
- **Back Button:** Navigate back to Product List page
...
- Image replacement option\n- **Vendor Update Section:**
  + Display current vendor information (Vendor Name, Contact Person, Phone)\n  + **Change Vendor button:** Opens vendor selection dialog
    * Option to select different existing vendor from dropdown
    * Option to create new vendor (same inline form as Add Product)
    * Confirm Change button: Updates vendor-product association
    * Cancel button: Retains current vendor\n  + **View Vendor Profile link:** Navigate to full vendor details in Vendor Management
- **Packaging Category Management:**
  + Display currently active packaging options with current stock levels
  + Option to add new packaging sizes (checkbox selection with stock quantity input)
  + Option to deactivate existing packaging sizes (deactivated packages hidden from customer view but stock data retained)
...
- Bulk actions: Update prices, Change category, **Assign Vendor**, Deactivate, Export to CSV
...
- **Table display with columns:**
  ...
  + Product Name
  + Category
  + **Packaging Size** (displays stock status for each packaging option separately)
...
**Update Stock Levels (Enhanced with Packaging Category Support)**
- **Back Button:** Navigate back to Stock Level Dashboard
- Product search and selection
- **Packaging Size Selection:** Dropdown to select specific packaging size (1kg, 2kg, 5kg, 10kg,200g, 500g, etc.)
- Current stock display for selected packaging size
...
- **Stock updates are recorded separately for each packaging size**
\n**Bulk Stock Update**
- CSV upload functionality for bulk updates
- Template download option (includes packaging size column)
- Validation and error reporting
- Preview before final update

**Low Stock Alerts**
- Configurable threshold settings per product or category
- **Separate alerts for each packaging size**
...
- **Back Button:** Navigate back to Stock Level Dashboard
- Table display: Date, Product, **Packaging Size**, Movement Type (In/Out), Quantity, Reason, Updated By, Notes
- Filter by date range, product, movement type, packaging size
- Export to CSV option
\n**Stock Reports**
- Current inventory status report (with packaging size breakdown)
- Stock movement report (date range selection)
- Low stock items report (by packaging size)
- Inventory valuation report (cost price × quantity for each packaging size)
...
#### 3.12.6 Vendor Management (Implementation Ready with Enhanced Purchase Order Integration)
...
**Mark Products as Received (Enhanced with Automatic Stock Update, Packaging Category Support, and Vendor Transaction Tracking)**
- **Back Button:** Navigate back to Purchase Order Details page
- **Receive Products Dialog:**
  + Product list with checkboxes for selection
  + **Packaging Size Selection:** Dropdown to select packaging size for each product (1kg, 2kg, 5kg, 10kg, 200g, 500g, etc.)
  + Quantity Received input field for each product and packaging size (defaults to ordered quantity)
...
- **Automatic Inventory Stock Update (Enhanced with Packaging Category Support):**
  + **Upon marking products as received, system automatically updates inventory stock levels by adding the received quantity to current stock for the selected packaging size**
  + **Stock Update Process:**
    * System identifies the product by Product ID and selected packaging size from the purchase order
    * Retrieves current stock quantity for the specific packaging size from inventory database
    * Adds received quantity to current stock: New Stock = Current Stock + Received Quantity
    * Updates product stock level for the specific packaging size in inventory management system
    * Displays confirmation message showing old stock, received quantity, and new stock level for the packaging size
  ...
  + **Stock Status Update:**
    * If product packaging size was previously Out of Stock or Low Stock, status automatically updates based on new stock level
    * Color-coded indicator changes accordingly (Red → Yellow → Green)
  + **Critical Stock Alert Removal:**
    * If product packaging size was listed in Critical Stock Status Dashboard and new stock level exceeds critical threshold, it is automatically removed from critical status list
- **Automatic Vendor Transaction Tracking:**
  + **Upon marking products as received, system automatically creates a transaction record in Vendor Profile:**
    ...
    * Products: List of received products with quantities and packaging sizes
...
- **Cancel button:** Closes dialog without saving\n- **Post-Receipt Confirmation:**
  + Success message displays: Products received successfully. Inventory updated: [Product Name] ([Packaging Size]) stock increased from [Old Stock] kg to [New Stock] kg. Vendor transaction recorded.
...
- **Product-wise Purchase Summary:** Total quantity ordered and received per product (with packaging size breakdown)
...
- Inventory turnover reports (with packaging size breakdown)
...
- **CriticalStockDashboard Component:** Centralized view of all critical stock items across categories with packaging size breakdown
...
- **PackagingSizeSelector Component:** Dropdown selector for choosing packaging size when updating stock or receiving purchase orders

#### 5.1.6 Product and Category Management Components (Enhanced)
...
- **ProductForm Component (Enhanced):** Enhanced product form with auto-generated Product ID field (read-only), category dropdown (selection only, no add category option), vendor selection/creation section, and packaging category selection with initial stock quantity input
- **PackagingCategorySelector Component:** Checkbox-based selector for choosing packaging options with stock quantity input fields for each selected package
- **VendorSelector Component (New):** Radio button selector for choosing between Existing Vendor or New Vendor, with dropdown for existing vendors and inline form for new vendor creation

#### 5.1.7 Vendor Management Components (New)
...
- **Add routes for packaging category management within product pages**
...
- **Integrate automatic stock update functionality with purchase order receipt process, supporting packaging category-specific updates**
- **Connect Critical Stock Status Dashboard with inventory database for real-time updates with packaging size breakdown**
...
- **Integrate vendor selection/creation with product management for automatic vendor-product linking**
...
- **Automatic stock update testing: Verify stock levels are correctly updated when purchase orders are marked as received, with separate tracking for each packaging size**
- **Critical stock dashboard testing: Ensure all products with critical stock levels are displayed accurately with packaging size breakdown**
...
- **Vendor selection and creation testing: Verify that vendor can be selected from existing vendors or created inline during product addition, and that vendor-product association is correctly established**
...
- **Purchase order and automatic stock update workflow documentation with packaging category support**
- **Critical stock monitoring and reordering guide with packaging size breakdown**
...
- **Vendor management guide: Instructions for adding vendors during product creation and managing vendor-product associations**
...
- **Vendor selection section integrated within product form with clear visual separation**
...
- **Vendor selection radio buttons with clear labels and visual feedback**
